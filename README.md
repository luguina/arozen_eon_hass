...
2. Unpack it and copy the **`custom_components/arozen_eon/` folder** out of the archive into your
   ...
   `config/custom_components/arozen_eon/manifest.json` exists. Twenty-two files land, including
   a `translations/` directory and the `brand/` images Home Assistant shows for the integration;
   nothing else in the archive is needed at runtime.
...
| ✅ | **A rotated local key no longer fails silently** (#49). Re-pairing in the app mints a new key, the old one stops decrypting immediately, and until now the entire user-facing consequence was `unavailable` entities and a climbing counter — the remedy existed and nothing pointed at it. An hour of silence now raises a repair card that names both plausible causes and asserts neither, because on this transport a dead key and a dead battery are the same error payload; the conventional `ConfigEntryAuthFailed` would have told users with an unplugged diffuser that their credentials were wrong ([ADR-008](docs/decisions.md#adr-008--never-assert-a-cause-the-transport-cannot-distinguish)). The threshold is wall clock rather than a count of polls, because the poll interval is a user setting spanning 10 s to 3600 s. **The card is now observed** (2026-08-29): the diffuser was taken off the network for an hour and it rendered as written — `Warning` rather than an error, both causes named and neither asserted, no fix button, and *Learn more* landing on the heading it targets. The run also measured something no test could. The card reported **29 consecutive attempts**, not the sixty a 60-second poll interval predicts, because tinytuya retries a dead socket five times at five-second intervals behind a 10 s timeout before it raises: a *failing* poll costs about a minute where a succeeding one costs milliseconds, and the coordinator schedules the next from the last one's completion. So the failure count tracks the library's retry loop rather than elapsed time — the argument for a wall-clock threshold made a second time, by measurement instead of by reasoning. That same loop sets how long the card outlives the fix: put back on the network, the diffuser cleared it unprompted a few minutes later, which is one failed cycle plus the time the device takes to rejoin — `_clear_unreachable_issue` retiring the card on the first exchange that works, poll or write, because either one proves the whole of what the card was claiming. |
...
| ✅ | **The HACS default-store gate is clear except for the submission itself.** Listing in `hacs/default` needs the HACS action to pass with **no ignores**, and this repository carried `ignore: brands` because the domain is registered in no brand repository. It no longer has to be: since Home Assistant 2026.3.0 a custom integration may carry its own brand images, and HACS accepts those in place of registration — so `custom_components/arozen_eon/brand/` now holds `icon.png` (the only required one) plus the `@2x` and `dark_` variants, cut from the vendor's own product photography, and the ignore is gone. Everything else the store asks for was already true: public, issues on, described, eight topics, two real releases, hassfest green, and a custom-repository install walked end to end. The one thing not yet proved is the action itself, which is guarded to the public repository and so cannot run here — it runs on the next publish, before the listing pull request is worth filing |
...
custom_components/arozen_eon/     the integration. dp.py is the only file that knows a DP
  ...
  diagnostics.py                  the ⋮ → Download diagnostics dump, with the local key,
                                  ...
                                  cannot see them
  brand/                          the icon and logo Home Assistant shows for the
                                  integration. icon.png is the only required one; the
                                  dark_ variants carry the silver unit, which reads on a
                                  dark theme where the black one goes flat
docs/
...
**The images in `custom_components/arozen_eon/brand/` are Arozen's**, not this project's — their
product photography and their wordmark, reproduced to identify the device the integration
controls and for no other purpose. The MIT licence on this repository covers the code; it does
not purport to grant anything in Arozen's marks, which remain theirs.
