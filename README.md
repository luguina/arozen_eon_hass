...
| ✅ | The **entity wiring** is verified too, in a throwaway Home Assistant driven over its own REST API: the config flow validates and creates the entry, exactly the eleven entities that were enabled by default *on that date* appear and nothing beyond them, and after `switch.turn_off` / `switch.turn_on` the intensity select still reads `L4 · every 10 min` with the restores sensor at 1 and `remembered_level: L4`. **23/23 on 2026-08-22**, and **23/23 again on 2026-08-28** — that second run against the **delivered file set** (16 files) rather than the working tree, which is the mode #63 added and the first time this harness tested what a user receives instead of what a developer has. It saw **ten** entities and not the eleventh, the recon sensor having moved to disabled-by-default in between (#51) |
